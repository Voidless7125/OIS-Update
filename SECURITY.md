...
| **SQLite** (`sqlite3.dll`) | 3.7.15.1 | **3.53.3** | Local Database Storage |
...
* **The Threat:** The original `zlib 1.2.5` is vulnerable to memory corruption bugs (such as the well-known CVE-2018-25032) when decompressing certain payloads. `SQLite 3.7.15.1` lacked modern memory safety and query sanitization features.
